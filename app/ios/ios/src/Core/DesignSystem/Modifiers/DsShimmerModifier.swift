...
//  ios
//
//  Created by Dmitrii Semenov on 12.02.2025.
//
...
public extension View {
    ...
    @ViewBuilder func shimmering(
        active: Bool = true,
        animation: Animation = DsShimmer.defaultAnimation,
        gradient: Gradient = DsShimmer.defaultGradient,
        bandSize: CGFloat = 0.3,
        mode: DsShimmer.Mode = .mask
    ) -> some View {
        if active {
            modifier(DsShimmer(animation: animation, gradient: gradient, bandSize: bandSize, mode: mode))
        } else {
            self
        }
    }
